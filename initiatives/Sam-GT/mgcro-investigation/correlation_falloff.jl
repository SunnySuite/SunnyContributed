...
function show_falloff!(ax,sc; cont = x -> x[1] + x[5] + x[9],slope = 0.0, scale = 1.0, log_mode = true, color = 0.5, kwargs...)
  ...
  if log_mode
    ...
    scatter!(ax,logdist,non_logcorr; color = color, colorrange = (0,1), kwargs...)
...
function sweep_afm_chain(;log_mode = true)
  Ts = 10 .^ range(-8,-1,length = 30)
  f = Figure(); ax = Axis(f[1,1])
  for (i,T) = enumerate(Ts)
    isc = example_falloff_system_afm_chain(;kT = T)
    show_falloff!(ax,isc; scale = log_mode ? 1/T : 1.0, log_mode, color = i/length(Ts), marker = :hline, colormap = :darkrainbow)
    display(f)
    sleep(0.01)
  end
  f
...
example_falloff_system_J1_pyrochlore_cl = begin
  pyrochlore = Crystal(I(3), [[1/2,0,0]],227,setting = "2")
  sys = System(pyrochlore, (3,3,3), [SpinInfo(1;S=1,g=2)],:dipole)
  #set_exchange!(sys,1.,Bond(1,2,[0,0,0]))
  set_pair_coupling!(sys,(Si, Sj) -> Si' * 1. * Sj - (Si' * 0.1 * Sj)^2,Bond(1,2,[0,0,0]))
...
function to_polarization_system(sys_pyro)
  pyrochlore = Crystal(I(3), [[1/2,0,0]],227,setting = "2")
  ...
  for j = 1:16
    ...
    closest_pass = diamond_chlore.latvecs * (mod.((diamond_chlore.latvecs \ raw_bond) .+ [0.5,0.5,0.5],[1.,1.,1.]) .- [0.5,0.5,0.5])
  ...
  for k = 1:8
    ...
    for ci = Sunny.eachcell(sys_pyro)
      for ni = 1:4
        ci_grab = CartesianIndex(mod1.(ci.I .+ cell_displace[k,ni],sys_pyro.latsize)...)
        sys_d.dipoles[ci,k] += sys_pyro.dipoles[ci_grab,neighs[ni]][3] * bond_vectors[neighs[ni]]
      end
    end
  end
  sys_d
...
function hyperplot_spins(sys;sigma = 1.0,n_spin = 1000,ntracer = 10,phase_mode = false,kwargs...)
  ...
  f_vec = function(pt)
    v = [0.,0.,0.]
    for cell in CartesianIndices(sys.latsize), a = 1:size(sys.dipoles,4)
      this_R = sys.crystal.latvecs * [cell[1] ; cell[2]; cell[3]] .+ atom_pos[a]
      this_spin = sys.dipoles[cell,a]
      closest_pass = supervecs * (mod.((supervecs \ (this_R .- pt)) .- [0.5,0.5,0.5],[1.,1.,1.]) .+ [0.5,0.5,0.5])
  ...
  for n = 1:ntracer
    ...
    lines!(ax,trail[1,:],trail[2,:],trail[3,:],linewidth = 3.5,color = :black)
    next!(prog)
  end
  finish!(prog)
  f
...
function sweep_J1_pyrochlore()
  ...
  for (i,T) = enumerate(Ts)
    nsamp = 16
    isc = example_falloff_system_J1_pyrochlore_cl(;kT = T,nsamp)
...
function glass_transition_J1_pyrochlore()
  ...
  for (i,T) = enumerate(Ts)
    ...
    for (i_t,tw) = enumerate(wander_times)
      ...
      isc = example_falloff_system_J1_pyrochlore_cl(;kT = T,nsamp,nwander = tw)
